...
def map(k,train_label, train_vec):
    ...
    pe1=[[0 for col in range(k+1)] for row in range(7)]
    pe0=[[0 for col in range(k+1)] for row in range(7)]
    for i in range(0,len(emotion)):
         #情绪label i
...
def mlKNN(k,a,b,dic_intention_polarity,dic_kafang,dic_smile):
    ...
    for test_weight in test_vec:
        test_emotion_temp=predict_test(test_weight,k,train_vec,ph,pe1,pe0,n,train_label)
